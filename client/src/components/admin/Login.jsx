...
//           </div>
//           <form onSubmit={handleSubmit} className='mt-6 w-full sm:max-w-md text-gray-600'></form>
...
const Login = () => {
  const { axios, setToken } = useAppContext(); // ✅ correct hook usage
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
    ...
    <div className='flex items-center justify-center h-screen'>
      <div className='w-full max-w-sm p-6 max-md:m-6 border border-primary/30'>
        ...
        <form onSubmit={handleSubmit} className='mt-6 w-full text-gray-600'>
          ...
          <button
            type='submit'
            className='w-full py-3 font-medium bg-primary text-white cursor-pointer hover:bg-primary/90'
          >
            Login
          </button>
        </form>
